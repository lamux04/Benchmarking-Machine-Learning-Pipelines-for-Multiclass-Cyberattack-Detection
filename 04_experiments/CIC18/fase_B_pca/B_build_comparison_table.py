"""Construye tablas comparativas globales de resultados B."""
...
def _find_report_files(suffix):
    """Localiza reportes B dentro de cualquier familia/modelo."""
    return sorted(RESULTS_DIR.glob(f"*/*/reports/*{suffix}"))


def _find_experiment_dirs():
    """Localiza directorios de experimento B."""
    return sorted(
        path
...
def _build_missing_reports_message(suffix):
    """Construye un diagnostico cuando aun no hay reportes B."""
    experiment_dirs = _find_experiment_dirs()

    if not experiment_dirs:
        return (
            "No se han encontrado experimentos B en resultados. "
            f"Ruta esperada: {RESULTS_DIR}/*/*"
        )

    lines = [
        "No se han encontrado reportes B para construir la comparativa.",
    ...
    lines.append(
        "Ejecuta primero los experimentos B; este script solo agrega los CSV "
        "que generan."
    )
...
def main():
    ...
    print("Tabla comparativa B generada.")
